...
async function run() {
  try {
    ...
    const reconcileCypher = `
      ...
      MATCH (nameOnly:Event) WHERE exists(nameOnly.name) AND (nameOnly.eventId IS NULL OR nameOnly.eventId = '')
      WITH nameOnly
      MATCH (idEvent:Event) WHERE idEvent.name = nameOnly.name AND idEvent.eventId IS NOT NULL
      WITH nameOnly, idEvent
      // Move relationships from nameOnly -> idEvent
      CALL {
        WITH nameOnly, idEvent
        // Update relationships outgoing from nameOnly to point to idEvent
        MATCH (nameOnly)-[r]->(target)
        WHERE NOT (idEvent)-[r2]->(target)
        WITH r, idEvent, target
        CREATE (idEvent)-[rcopy:TYPE(r)]->(target)
        SET rcopy += r
        RETURN count(rcopy) as movedOut
      }
      CALL {
        WITH nameOnly, idEvent
        // Update relationships incoming to nameOnly to point to idEvent
        MATCH (source)-[r]->(nameOnly)
        WHERE NOT (source)-[r2]->(idEvent)
        WITH r, idEvent, source
        CREATE (source)-[rcopy:TYPE(r)]->(idEvent)
        SET rcopy += r
        RETURN count(rcopy) as movedIn
      }
      // Copy properties that idEvent is missing
      WITH nameOnly, idEvent
      CALL {
        WITH nameOnly, idEvent
        WITH apoc.map.clean(apoc.map.merge(idEvent, nameOnly), [''], []) AS mergedProps
        SET idEvent += mergedProps
        RETURN idEvent
      }
      // Remove the old node and its dangling relationships
      WITH nameOnly
