/**
 * 🛡️ ELITE BOT SHIELD
 * Micro-service Gateway Security Layer
 */
...
const userAgent = (req.headers['user-agent'] || '').toLowerCase();
    ...
    if (now - history.lastHit > 10000) {
        ...
        history.count++;
        // If > 20 requests per 10s window (2 req/sec sustained)
        if (history.count > 20) {
    ...
    // 3. Behavioral Scoring (Add header for downstream services)
    // Downstream services can use this to enforce harder checks (e.g. CAPTCHA)
    let botScore = 0;
