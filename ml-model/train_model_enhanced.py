...
class EnhancedTicketPricingModel:
    ...
    def generate_enhanced_synthetic_data(self, n_samples=15000):
        ...
        event_duration_days = np.random.randint(
            1, 30, n_samples
        )  # Duration from 1 to 30 days
        event_popularity = np.random.beta(
            2, 3, n_samples
        )  # Skewed toward lower popularity
        ...
        season = np.random.randint(
            1, 5, n_samples
        )  # 1=Winter, 2=Spring, 3=Summer, 4=Monsoon
        ...
        venue_tier = np.random.randint(
            1, 4, n_samples
        )  # 1=Small, 2=Medium, 3=Large/Stadium
        artist_tier = np.random.randint(
            1, 6, n_samples
        )  # 1=Local, 5=International Star
        ...
        event_duration_factor = (
            1 + (event_duration_days / 10) * 0.1
        )  # Multi-day events get premium
    ...
    def create_model(self):
        """Create an optimized, regularized XGBoost model to prevent overfitting"""
        print("Initializing Regularized XGBRegressor (XGBoost)...")
        if not HAS_XGBOOST:
    ...
    def train(self, df, use_cross_validation=True):
        """Train the pricing model with cross-validation and overfitting diagnosis"""
        ...
        metrics = {
            ...
            "overfitting_status": "OVERFITTING DETECTED" if is_overfitting else "NO OVERFITTING (EXCELLENT GENERALIZATION)"
        }

        print("\n=== OVERFITTING DIAGNOSTIC REPORT ===")
        print(f"   Training R2 Score:   {r2_train:.4f}")
        print(f"   Testing R2 Score:    {r2_test:.4f}")
        print(f"   R2 Gap (Train-Test): {r2_gap:.4f} ({r2_gap*100:.2f}%)")
        print(f"   Training MAE:        INR {mae_train:.2f}")
        print(f"   Testing MAE:         INR {mae_test:.2f}")
        print(f"   Overfitting Status:  {'OVERFITTING DETECTED' if is_overfitting else 'PASSED: NO OVERFITTING (Model generalizes well!)'}")

        print("\n=== Performance Metrics ===")
        print(f"   Training R2 Score: {metrics['train_r2']:.4f}")
        print(f"   Testing R2 Score:  {metrics['test_r2']:.4f}")
        print(f"   Training MAE:      INR {metrics['train_mae']:.2f}")
        print(f"   Testing MAE:       INR {metrics['test_mae']:.2f}")
        print(f"   Training RMSE:     INR {metrics['train_rmse']:.2f}")
        print(f"   Testing RMSE:      INR {metrics['test_rmse']:.2f}")
    ...
    def load_model(
        self, model_path="model_enhanced.pkl", scaler_path="scaler_enhanced.pkl"
    ):
...
def main():
    ...
    # Save model
    print("\nSaving model...")
    pricing_model.save_model(model_path="model.pkl", scaler_path="scaler.pkl")

    # Test predictions with various scenarios
    print("\nTesting predictions with real-world scenarios...")
    test_cases = [
        (
            "Budget Event (Low Demand)",
            [100, 1000, 60, 1, 0.3, 200, 50, 2, 3, 14, 0, 0, 1, 2],
        ),
        (
            "Premium Concert (3-day)",
            [2500, 3000, 3, 3, 0.9, 2000, 1500, 4, 6, 20, 1, 0, 3, 5],
        ),
        (
            "Weekend Holiday Show (2-days)",
            [800, 1500, 15, 2, 0.6, 800, 400, 1, 7, 19, 1, 1, 2, 3],
        ),
        (
            "Last Minute Hot Event (1-day)",
            [1200, 2000, 1, 1, 0.85, 1500, 800, 3, 5, 21, 0, 0, 2, 4],
        ),
    ]
    ...
    for name, features in test_cases:
        ...
        print(f"\n   Scenario: {name}:")
        print(f"      Predicted Price: INR {result['price']:.2f}")
    ...
    model_info = {
        ...
        "r2Gap": float(metrics["r2_gap"]),
        "isOverfitting": bool(metrics["is_overfitting"]),
        ...
        "metadata": {"trainedAt": datetime.now().isoformat(), "currency": "INR"},
    }
    ...
    try:
        response = requests.post(
            f"{backend_url}/api/ml-model/update-metadata", json=model_info, timeout=5
        )
        if response.status_code == 200:
            print(f"   Successfully synced Version {model_version} to MongoDB")
        else:
            print(
                f"   Sync skipped (backend returned status: {response.status_code})"
            )
    except requests.exceptions.ConnectionError:
        print("   Backend not running - skipping MongoDB sync (this is OK)")
    ...
    print(
        f"   Test R2 Score: {metrics['test_r2']:.4f} ({metrics['test_r2'] * 100:.2f}%)"
    )
    print(f"   Test MAE: INR {metrics['test_mae']:.2f}")
    print(f"   Overfitting Check: {'OVERFITTING DETECTED' if metrics['is_overfitting'] else 'PASSED (No Overfitting)'}")
    print("   Files: model.pkl, scaler.pkl, model_info.json")
    print("")
